...
class Dataset(data.Dataset):
    def __init__(self, root_dir, split, bit=11, maximum=20475, type='train', scaling_factor=1, time_step=1, format='ply'):
        ...
        for i in split:
            ...
            sequence_dir = join(root_dir, sequence_prefix[i]+'_vox'+str(bit))
    ...
    def __getitem__(self, item):
        ...
        if self.format == 'npy':
            ...
            assert(os.path.exists(file_dir)), 'cannot find file: ' + file_dir
            assert(os.path.exists(file_dir1)), 'cannot find file: ' + file_dir1
            p = np.asarray(open3d.io.read_point_cloud(file_dir, remove_nan_points=True, remove_infinite_points=True, print_progress=True).points)
...
if __name__ == '__main__':
    d = Dataset(root_dir = '/tmp/code/d-dpcc/dataset', split=[0], type='train', format='ply')
    print(d[0])